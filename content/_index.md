---
title: "eralp.dev"
intro: |
  I'm a Senior Software Engineer at **Starship Technologies** in Helsinki, Finland. I've been building software since 2008. I ship end-to-end, from prototype to production, solo or with a team.

  [Get in touch](mailto:eralp@eralpkaraduman.com) | [CV](https://eralpkaraduman.github.io/cv/)
---

Currently working on mobile & web apps at Starship. My main stack is **React Native**, **React**, **Node.js**, **GraphQL**, and **TypeScript**. I also write native **Android/iOS bridges** for React Native, from Bluetooth hardware integrations to MDM tooling. Previously shipped iOS apps with **Swift**, and cross-platform apps with **Flutter** and **React Native**.

I have solid infrastructure experience from self-hosted setups on **Hetzner** with **Coolify** to **Cloudflare** edge services (Pages, Workers, D1, R2, Tunnels) and deploying to **Kubernetes** in production. I have extensive experience of **GCP**.

I build side projects ([GitHub](https://github.com/eralpkaraduman)) such as 3D web apps (**Three.js**, **react-three-fiber**, **Blender**) and Chrome extensions to hardware hacks and electronics. I also build with **AI tools** and integrate LLM inference APIs into projects.

I've also produced a comedy podcast ([Kulak Show](https://open.spotify.com/show/6Ace2VvsbkPKFkgqy4NWUc?si=0097ff13ea40460d)) and used to live-stream coding on [Twitch](https://twitch.tv/erikThePlum).

If you're bootstrapping your startup, [get in touch](mailto:eralp@eralpkaraduman.com) — I can help figure out tech architecture, infrastructure, and finding the right people.
